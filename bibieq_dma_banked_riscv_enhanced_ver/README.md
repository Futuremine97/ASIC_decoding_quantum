...
## Enhanced Documentation Pack (Professor-Style)

- `docs/spec.md` - system memory map, CSR map, CDC rules
- `docs/verification_plan.md` - coverage goals and CDC checklist
- `docs/perf_accuracy.md` - performance/accuracy methodology and template
...
- `rtl/csr_map.vh` (single source of CSR offsets/bits)
...
- `rtl/apb_regs.v` - APB3 register block
- `rtl/ahb_lite_regs.v` - AHB-Lite register block
...
- `rtl/bibieq_dma_banked_riscv_apb_top.v` - SoC-ready top (APB3)
- `rtl/bibieq_dma_banked_riscv_ahb_top.v` - SoC-ready top (AHB-Lite)
...
Reference C scaffold is under `sw/` (see `sw/README.md`). It mirrors the CSR map and system address map in `docs/spec.md`.
